...
const config = {
    ...
    sushi: '',
    alb: '',
    lfgWethLp: '0xbc09b31f5d2eb1dcfa5832f37d895dc00324fec4',
    usdcWethPairAddress: ''
}
...
async function main() {
    ...
    /* deposit WETH
    console.log('deposing WETH to farm')
    await weth.deposit({value: amount});
    ...
    await masterChef.deposit(2, amount, 0, [])
    */

    /* swap for USDC
    await router.swapExactETHForTokens(0, [config.weth, config.usdc], deployer.address, 1787246406, { value: amount.toString()})
    const usdcBalance = await usdc.balanceOf(deployer.address)
    console.log(`usdc balance: ${ethers.utils.formatUnits(usdcBalance, 6)}`)
    */

    /*
    console.log('adding liquidity to LFG-WETH')
    await router.addLiquidityETH(token.address, amount, 0, 0, deployer.address, 1787246406, {value: amount.div(1800)})

    const lfgWeth = await ethers.getContractAt("MockToken", config.lfgWethLp)
    console.log("TS", ethers.utils.formatEther(await lfgWeth.totalSupply()))

    console.log("MC balance of LFG: ", ethers.utils.formatEther(await token.balanceOf(masterChef.address)));
    console.log("MC balance of WETH:", ethers.utils.formatEther(await weth.balanceOf(masterChef.address)));
    */
